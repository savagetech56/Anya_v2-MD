...
const extractAnyaPlugins = (code) => {
    ...
    while ((match = regex.exec(code)) !== null) {
        try {
            // Use try-catch to ensure safety during eval
...
const installPlugins = async (rawUrl) => {
    ...
    try {
        ...
        // Ensure the script runs and compiles correctly
        try {
            new vm.Script(code, { filename });  // Validate syntax only
        } catch (syntaxError) {
            return {
                status: 400,
                statusEmoji: "⚠️",
        ...
        for (const plugin of plugins) {
            ...
            // Handle the `notCmd` field logic
            if (plugin.notCmd) {
                plugin.name = false;  // If `notCmd` is true, set name to false
            }
        ...
        return {
            ...
            plugins: plugins.map(plugin => ({
                name: plugin.name || 'N/A',  // Ensure there's a name (fallback if notCmd)
                filename,
                url,
            })),
        };
