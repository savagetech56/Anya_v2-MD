...
anya(
	...
	async (anyaV2, pika) => {
		try {
			...
			await anyaV2.sendMessage(pika.chat, {
				image: await getBuffer(`https://pikabotzapi.vercel.app/maker/genuptime/?apikey=anya-md&days=${uptime.days}&hours=${uptime.hours}&minutes=${uptime.minutes}&seconds=${uptime.seconds}&title=SYSTEM%20UPTIME&colour=cyan&background=black&glow=true&glowtype=2&glowlevel=5`),
				caption: `My Uptime : _${formatRuntime(uptimeInSeconds).trim()}_`
			}, { quoted: pika });
		} catch {
...
anya({
    ...
    try {
        ...
        await pika.reply("_🦋Bot restarted successfully!_");
    } catch (error) {
