...
anya(
	...
	async (anyaV2, pika, { args, prefix, command }) => {
		...
		if (args.join(" ").includes('spotify.com')) return pika.reply(`Url not allowed! for Urls use _${prefix + command}dl_ for urls`);
	        ...
	        .then(({data}) => {
			if (data.code !== 200 && !data.status) return pika.reply(code.message);
			let caption = '';
			...
			for (const i of data.results) {
				...
				caption += `> _*Album :*_\n`;
				caption += `>   - _name :_ ${i.album.name}_\n`;
				caption += `>   - _type :_ ${i.album.type}_\n`;
				caption += `>   - _artist :_ ${i.album.artist}_\n\n\n`;
			}
			caption += `> _ID: QA49_`;
			pika.reply(caption);
		});
