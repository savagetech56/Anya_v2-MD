...
anya(
	...
	async (anyaV2, pika, { args, prefix, command }) => {
		...
		if (args.join(" ").includes('spotify.com')) return pika.reply(`Url not allowed! for Urls use _${prefix + command}dl_ for urls`);
	        axios.get('https://api.giftedtech.my.id/api/search/spotifysearch?apikey=gifted-md&query=' + encodeURIComponent(args.join(" ")))
	        .then(({data}) => {
			if (data.status !== 200 && data.success) return pika.reply('_Oops! I got an issue!_');
			let caption = '';
			caption += '🏮 `Reply a number to download that song!`\n🏮 *Example :* 3\n\n';
			let count = 0;
			for (const i of data.results?.data) {
				count++;
				caption += `*[${count}] ${i.title}*\n`;
				caption += `_duration : ${i.duration} minutes_\n`;
