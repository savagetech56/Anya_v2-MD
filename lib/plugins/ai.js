...
anya(
	...
	alias: ['craiyongen'],
        ...
        need: 'query',
        category: 'ai',
        desc: 'Generate AI crayon images using craiyon.com ai',
        cooldown: 8,
        filename: __filename
    },
    async (anyaV2, pika, { args }) => {
        if (args.length < 1) return pika.reply("_What image you wish darling!?_");
        const MAX_IMAGES = 3;
	const msgKey = await pika.keyMsg(Config.message.wait + '\n> It\'t take longer time');
	craiyonImgGen(args.join(" "), { model: 'v4' })
            .then(async response => {
                if (!response.status || response.images.length < 1) return pika.reply("A-ah darling I didn't find any image...");
                let count = 0;
		...
		await pika.deleteMsg(msgKey.key);
            });
    }
);
...
anya(
	...
	alias: ['craiyonimg'],
        ...
        need: 'query',
        category: 'ai',
        desc: 'Search AI generated crayon images from craiyon.com',
        cooldown: 8,
        filename: __filename
    },
    async (anyaV2, pika, { args }) => {
        if (args.length < 1) return pika.reply("_What image you wish darling!?_");
        const MAX_IMAGES = 3;
	axios.get(`https://api2.craiyon.com/search?text=${encodeURIComponent(args.join(" "))}&max_results=${MAX_IMAGES}`)
            .then(async ({ data }) => {
                if (data.length < 1) return pika.reply("A-ah darling I didn't find any image...");
                for (const item of data) {
                    await anyaV2.sendMessage(pika.chat, {
                        image: { url: item.url },
                        caption: `*${item.alt}*

*Date :* _${item.date_created}_
`.trim()
                    }, { quoted: pika });
                }
            });
...
anya(
    {
        ...
        need: 'query',
        category: 'ai',
        desc: 'Search AI generated images from freepik.com',
        cooldown: 8,
        filename: __filename
    },
    async (anyaV2, pika, { args }) => {
        if (args.length < 1) return pika.reply("_What image you wish darling!?_");
        const MAX_IMAGES = 3;
        axios.get(`https://www.freepik.com/api/regular/search?filters[ai-generated][only]=1&filters[content_type]=photo&locale=en&page=${Math.floor(Math.random() * 100) + 1}&term=${encodeURIComponent(args.join(" "))}`)
            .then(async ({ data }) => {
                if (!data.items || data.items?.length < 1) return pika.reply("A-ah darling I didn't find any image...");
                const selectedItems = data.items
                    .sort(() => 0.5 - Math.random())
                    .slice(0, MAX_IMAGES);
                for (const item of selectedItems) {
                    await anyaV2.sendMessage(pika.chat, {
                        image: { url: item.preview.url },
                        caption: item.name
                    }, { quoted: pika });
                }
            });
