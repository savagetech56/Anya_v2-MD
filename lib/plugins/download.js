...
async (anyaV2, pika, { args, prefix, command }) => {
    ...
    let response = await saveTheVideo(args[0]); console.log(response);
    if (!response.status || response.result.length < 1) {
        pika.edit("API 1 failed, retrying with API 2...", keyMsg.key);
        response = await igdl(args[0]); console.log(response);
    ...
    for (let i = 0; i < response.result.length; i++) {
        await anyaV2.sendFileUrl(pika.chat, {
            url: response.result[i].formats.filter(v => v.ext === 'mp4').pop().url,
            caption: `
${response.result[0]?.description}

> ⏳ ${response.result[0]?.duration} seconds
> 👍🏻 ${formatNumber(response.result[0]?.like_count)}
> 🗨️ ${formatNumber(response.result[0]?.comment_count)}
            `.trim(),
