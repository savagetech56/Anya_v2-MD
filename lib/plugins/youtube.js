...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        const random = pickRandom(search.slice(0, 8));
        pika.reply(`*Downloading • _${random.title.replace(/\*/g, "")}_*`);
        const download = await youtube.downloadYtVideo(random.url);
        console.log(download.video);
        console.log(download.video.find(v => v.quality === 'auto'));
        //console.log(download.video.find(v => v.quality === 'auto')?.
        if (!download.status) return pika.reply("```⚠️ API ERR```");
        const ui = db.UI?.[0] || await new UI({ id: "userInterface" }).save();
        const cpt = [
            download.likes ? `⌈Lɪᴋᴇs⌋ : ${download.likes}` : null,
            random.views || download.views ? `⌈Vɪᴇᴡs⌋ : ${formatNumber(random.views || download.views)}` : null,
            random.seconds || download.duration ? `⌈Dᴜʀᴀᴛɪᴏɴ⌋ : ${formatRuntime(random.seconds || download.duration)}` : null,
            random.ago || download.uploadedOn ? `⌈UᴘʟᴏᴀᴅᴇᴅOɴ⌋ : ${random.ago || download.uploadedOn}` : null,
            random.author?.url || download.channel ? `⌈Cʜᴀɴɴᴇʟ⌋ : ${random.author?.url?.split("https://youtube.com/")[1] || download.channel}` : null,
            random.url || download.url ? `⌈Uʀʟ⌋ : ${random.url || download.url}` : null
        ].filter(Boolean).join("\n");
        const caption = `\`\`\`\n${random.title || download.title || "NO_TITLE_FOUND"}\n\n${cpt}\n\`\`\``.trim();      
        if (download.response_type === 'default') {
            const videoBuffer = Buffer.isBuffer(download.video) ? download.video : { url: download.video };
            if (ui.buttons) {
        ...
        if (download.response_type === 'array') {
            const videoButtons = download.video.map(v => ({
                name: "cta_url",
                buttonParamsJson: `{"display_text":"${v.quality} • ${v.size || '?? MB'}","url":"${v.dl_url}","merchant_url":"${v.dl_url}"}`
            }));
            if (ui.buttons) {
                return anyaV2.sendButtonImage(pika.chat, {
                    image: { url: random.thumbnail || download.thumbnail || Config.imageUrl },
                    caption: `${caption}\n\n\`Tap on the buttons below to download/stream videos\``,
                    footer: Config.footer,
                    buttons: videoButtons
                }, { quoted: pika });
            }
            const urlAndQuality = download.video.map(v => `
*🌟 ${/720p|1080p/.test(v.quality) ? v.quality : `${v.quality} _(not recommended)_`}*
> Size : ${v.size || '?? MB'}
> Url : ${v.shorturl}`.trim()).join("\n\n");
            return anyaV2.sendMessage(pika.chat, {
                text: `
*\`Tap on the links below to stream/download the video\`*
...
`.trim(),
                contextInfo: {
                    externalAdReply: {
                        showAdAttribution: true,
                        title: random.title || download.title || "NO_TITLE_FOUND",
                        body: 'Tap here to watch video!',
                        thumbnailUrl: random.thumbnail || download.thumbnail || Config.imageUrl,
                        mediaType: 1,
                        renderLargerThumbnail: true,
                        sourceUrl: download.video.find(v => v.quality === 'auto')?.[0].dl_url,
                        mediaUrl: download.video.find(v => v.quality === 'auto')?.[0].dl_url
                    }
                }
            }, { quoted: pika });
        }
