...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        const search = await youtube.searchV2(query, "videos", 3);
        if (search.length < 1) return pika.reply("_Video Not Found!_");
        const random = pickRandom(search);
        pika.reply(`*Downloading • _${random.snippet?.title?.replace(/\*/g, "") || random.title?.replace(/\*/g, "") || 'NO_TITLE_FOUND'}_*`);
        const download = await youtube.downloadYtVideo(random.url || (random.id?.videoId ? `https://youtube.com/watch?v=${random.id.videoId}` : null)); //console.log(download);
        ...
        if (random.snippet?.publishTime || random.ago || download.uploadedOn) cpt.push(`⌈UᴘʟᴏᴀᴅᴇᴅOɴ⌋ : ${random.snippet?.publishTime || random.ago || download.uploadedOn}`);
        if (random.snippet?.channelTitle || random.author?.url || download.channel) cpt.push(`⌈Cʜᴀɴɴᴇʟ⌋ : ${random.snippet?.channelTitle || random.author?.url?.split("https://youtube.com/")[1] || download.channel}`);
        if (random.id?.videoId || random.url || download.url) cpt.push(`⌈Uʀʟ⌋ : ${random.url || download.url || (random.id?.videoId ? `https://youtube.com/watch?v=${random.id.videoId}` : null)}`);
        const caption = `
\`\`\`
${random.snippet?.title || random.title || download.title || "NO_TITLE_FOUND"}

${cpt.join("\n")}
\`\`\`
        `.trim();
        let buffer = Buffer.isBuffer(download.video) ? download.video : { url: download.video };
        if (ui.buttons) {
...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        const random = pickRandom(search);
        pika.reply(`*Downloading • _${random.snippet?.title?.replace(/\*/g, "") || random.title?.replace(/\*/g, "") || 'NO_TITLE_FOUND'}_*`);
        const download = await youtube.downloadYtAudio(random.url || (random.id?.videoId ? `https://youtube.com/watch?v=${random.id.videoId}` : null));
        if (!download.status) return pika.reply("```⚠️ API ERR```");
        const cpt = [];
        ...
        if (random.snippet?.channelTitle || random.author?.url || download.channel) cpt.push(`${random.snippet?.publishTime || random.ago || download.uploadedOn}`);
        let buffer;
        if (Buffer.isBuffer(download.audio)) {
            buffer = download.audio;
        } else {
            const tempDir = path.join(__dirname, '../../.temp');
            const outputFilePath = path.join(tempDir, getRandom(8));
            const file = await new Promise((resolve, reject) => {
                ffmpeg(download.audio)
...
anya(
    ...
    async (anyaV2, pika, { db, args, prefix, command }) => {
        ...
        const random = pickRandom(search);
        pika.reply(`*Downloading • _${random.snippet?.title?.replace(/\*/g, "") || random.title?.replace(/\*/g, "") || 'NO_TITLE_FOUND'}_*`);
        const download = await youtube.downloadYtVideo(random.url || random.id?.videoId ? `https://youtube.com/watch?v=${random.id.videoId}` : null);
        if (!download.status) return pika.reply("```⚠️ API ERR```");
        const cpt = [];
        if (download.likes) cpt.push(`⌈Lɪᴋᴇs⌋ : ${download.likes}`);
        if (random.views || download.views) cpt.push(`⌈Vɪᴇᴡs⌋ : ${random.views ? formatNumber(random.views) : formatNumber(download.views)}`);
...
\`\`\`
        ...
        anyaV2.sendMessage(pika.chat, {
            document: buffer,
            caption: caption + `\n> ${Config.footer}`,
            fileName: (random.id?.videoId || random.videoId || download.id) + '.mp4',
            mimetype: "video/mp4",
            //contextInfo: {
            //    externalAdReply: {
...
\`\`\`
        ...
        anyaV2.sendMessage(pika.chat, {
            document: buffer,
            caption: caption + `\n> ${Config.footer}`,
            fileName: (random.id?.videoId || random.videoId || download.id) + '.mp3',
            mimetype: "audio/mp3",
            //contextInfo: {
            //    externalAdReply: {
            //        title: download.title || random.title,
